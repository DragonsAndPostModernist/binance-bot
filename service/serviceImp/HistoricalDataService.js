    ...
    async execute( candles, interval, pair, binance ){
        ...
        if( this.pollTime <= POLL_INTERVAL ){
            ...
            for (let index = 0; index < len; index++) {
                process.stdout.write(`${chalk.cyan("Fetching  Historical Data from Exchange ....")}  ${chalk.yellow(`Processed ${chalk.green(index + 1 )} records.  ${chalk.red(candleSticks.getCandles().candles.length - (index +1) )} left to process`)}`+ '\r')
            }
